...
export class BusinessRulesService {
   ...
   */
  validateProposal(proposalData: any, produto: Produto, tabelaComercial: TabelaComercial): ValidationResult {
    const errors: string[] = [];
  ...
  private validateClientData(clienteData: any): ValidationResult {
    ...
    if (clienteData.rendaMensal) {
      ...
      if (!renda || renda.lessThan(Money.fromReais(1320))) { // Current minimum wage
    ...
    if (!condicoesData.prazo || condicoesData.prazo <= 0) {
      ...
      if (tabelaComercial.prazos && tabelaComercial.prazos.length > 0) {
        if (!tabelaComercial.prazos.includes(prazo)) {
          warnings.push(`Prazo ${prazo} não está disponível na tabela comercial. Prazos disponíveis: ${tabelaComercial.prazos.join(', ')}`);
        }
      }
    }
  ...
  private validateCrossFieldRules(proposalData: any, produto: Produto): ValidationResult {
    ...
    if (clienteData.dataNascimento && condicoesData.prazo) {
      ...
      if (idadeFinalContrato > 75) {
        warnings.push('Cliente terá mais de 75 anos ao final do contrato - avaliar capacidade futura');
      }
    }
   ...
   */
  private validateEmprestimoPessoalRules(proposalData: any, errors: string[], warnings: string[]): void {
    const condicoesData = proposalData.condicoes_data;
  ...
  private validateCreditoRotativoRules(proposalData: any, errors: string[], warnings: string[]): void {
    const condicoesData = proposalData.condicoes_data;
  ...
  private validateFinanciamentoVeiculoRules(proposalData: any, errors: string[], warnings: string[]): void {
  ...
  private validateEmprestimoGarantiaRules(proposalData: any, errors: string[], warnings: string[]): void {

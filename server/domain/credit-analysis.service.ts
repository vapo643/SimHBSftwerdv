...
export class CreditAnalysisService {
  ...
  async analyzeCredit(input: CreditAnalysisInput): Promise<CreditAnalysisResult> {
    ...
    const productValidation = this.validateProductLimits(valorSolicitado, input.prazoSolicitado, input.produto);
    ...
    if (aprovado || score >= 400) { // Partial approval
  ...
  private calculateIncomeScore(rendaMensal: number | undefined, valorSolicitado: Money): number {
    ...
    if (compromissoRenda <= 0.50) return 150; // <= 50% of income
   ...
   */
  private calculateDebtScore(dividas: Divida[] | undefined, rendaMensal: number | undefined): number {
    ...
    if (comprometimentoRenda <= 0.20) return 200; // <= 20% debt/income
    if (comprometimentoRenda <= 0.35) return 150; // <= 35% debt/income
    if (comprometimentoRenda <= 0.50) return 100; // <= 50% debt/income
    if (comprometimentoRenda <= 0.70) return 50;  // <= 70% debt/income
   ...
   */
  private applyRiskModifiers(input: CreditAnalysisInput, baseScore: number): {
  ...
  generateAnalysisSummary(input: CreditAnalysisInput, result: CreditAnalysisResult): string {
    ...
    return `Análise de Crédito - ${status} | CPF: ${input.clienteCpf} | ` +

  ...
  try {
    ...
    res.json({
      ...
      analysis: {
        ...
        missingData: logs.filter(l => l.includes('Sem valor')).map(l => {
  ...
  try {
    const mapping = {
      ...
      page2: {
        ...
        fields: {
          numeroBancoEmitente: { source: 'EXTRAÍDO de dadosPagamentoBanco', status: '✅ CORRIGIDO' },
          contaNumeroEmitente: { source: 'dadosPagamentoAgencia + dadosPagamentoConta', status: '✅ CORRIGIDO' },
          nomeInstituicaoFavorecida: { source: 'proposta.dadosPagamentoBanco' },
          numeroContrato: { source: 'proposta.id' },
          linhaDigitavelBoleto: { source: 'inter_collections.linhaDigitavel', status: '⚠️ INTEGRAÇÃO' }
        }
      },
      page8: {
  ...
  try {
    ...
    const completeness = Math.round(
      ((Object.values(requiredFields).filter(Boolean).length + 
       ...
       (Object.keys(requiredFields).length + Object.keys(importantFields).length)) * 100
    );

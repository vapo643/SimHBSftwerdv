        ...
        try {
          ...
          if (pdfBuffer && pdfBuffer.length > 0) {
            ...
            if (pdfMagic.startsWith("%PDF")) {
              ...
              if (result.success) {
                console.log(`[PAM V1.0 SYNC] ✅ Boleto ${collection.codigoSolicitacao} salvo no Storage`);
                boletosProcessados++;
              } else {
                console.error(`[PAM V1.0 SYNC] ❌ Erro ao salvar boleto ${collection.codigoSolicitacao}:`, result.error);
                erros++;
              }
            } else {
              console.error(`[PAM V1.0 SYNC] PDF inválido para boleto ${collection.codigoSolicitacao}`);
              erros++;
            }
          } else {
          ...
          console.error(`[PAM V1.0 SYNC] Erro ao processar boleto ${collection.codigoSolicitacao}:`, error);
          erros++;
        }
      }
      
      console.log(`[PAM V1.0 SYNC] Sincronização concluída: ${boletosProcessados} processados, ${erros} erros`);

 ...
 */
router.get("/:id", jwtAuthMiddleware, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.put("/:id/reactivate", jwtAuthMiddleware, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const statusCode = error instanceof Error && 
      error.message.includes("já está ativo")
      ? 400 : 500;
 ...
 */
router.get("/role/:role", jwtAuthMiddleware, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.error(`❌ [Controller/Users] Error fetching users by role ${req.params.role}:`, error);
    res.status(500).json({ 
 ...
 */
router.get("/loja/:lojaId", jwtAuthMiddleware, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.error(`❌ [Controller/Users] Error fetching users by loja ${req.params.lojaId}:`, error);
    res.status(500).json({ 

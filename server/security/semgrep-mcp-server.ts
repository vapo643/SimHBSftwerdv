...
export class SemgrepMCPServer {
  ...
  async getComponentContext(component: string): Promise<ComponentSecurityContext> {
    ...
    const results = await Promise.all(
      componentFiles.map(file => this.scanFile(file))
    )
  ...
  private async processResults(rawResults: any, scanId: string): Promise<SemgrepResult> {
    const findings: SecurityFinding[] = rawResults.results?.map((result: any) => ({
      ...
      context: {
        before: result.extra.lines?.before || [],
        match: result.extra.lines?.text || '',
        after: result.extra.lines?.after || []
      }
    })) || []
   ...
   */
  private async enrichWithContext(result: SemgrepResult, context: AnalysisContext): Promise<SemgrepResult> {
  ...
  private calculateAttackSurface(results: SemgrepResult[]): any {
    return {
      exposed_endpoints: 0,
      authentication_issues: results.filter(r => r.findings.some(f => f.rule_id.includes('auth'))).length,
      input_validation_issues: results.filter(r => r.findings.some(f => f.rule_id.includes('injection'))).length
    }
  }

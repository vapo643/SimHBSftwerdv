...
const NameSchema = z.string().min(3).max(255).transform(val => xss(val));
...
class ClickSignSecurityService {
  ...
  checkWebhookRateLimit(ip: string): boolean {
    ...
    if (this.webhookAttempts.size > 1000) {
      const oldestIP = Array.from(this.webhookAttempts.entries())
        .sort((a, b) => Math.max(...a[1]) - Math.max(...b[1]))[0][0];

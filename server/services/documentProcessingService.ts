...
export class DocumentProcessingService {
  ...
  async processSignedDocument(
    ...
    try {
      console.log(`📄 [DOCUMENT PROCESSING] Starting processing for proposal ${proposalId} via ${source}`);
      ...
      const clickSignDocId = documentKey || proposal.clicksign_document_id || proposal.clicksign_envelope_id;

      if (!clickSignDocId) {
        console.warn(`⚠️ [DOCUMENT PROCESSING] No ClickSign document ID for proposal ${proposalId}`);
        return {
          success: false,
      ...
      if (proposal.caminho_ccb_assinado) {
        ...
        if (fileExists && fileExists.length > 0) {
          console.log(`✅ [DOCUMENT PROCESSING] Document already processed for proposal ${proposalId}`);
          return {
            success: true,
      ...
      if (source === ProcessingSource.WEBHOOK) {
        console.log(`✅ [DOCUMENT PROCESSING] Document for proposal ${proposalId} processed via WEBHOOK in ${processingTime}ms`);
      } else if (source === ProcessingSource.POLLING) {
        console.warn(`⚠️ [DOCUMENT PROCESSING] Document for proposal ${proposalId} processed via POLLING FALLBACK in ${processingTime}ms`);
      } else {
        console.log(`✅ [DOCUMENT PROCESSING] Document for proposal ${proposalId} processed via ${source} in ${processingTime}ms`);
      }
      ...
      console.error(`❌ [DOCUMENT PROCESSING] Error processing document for proposal ${proposalId}:`, error);
  ...
  async processBatch(
    ...
    console.log(`🔄 [DOCUMENT PROCESSING] Processing batch of ${proposals.length} documents via ${source}`);

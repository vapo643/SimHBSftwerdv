...
export async function logStatusTransition(transition: StatusTransitionLog) {
  try {
    console.log(`[AUDIT V2.0] 📝 Registering status transition for proposal ${transition.propostaId}`);
    console.log(`[AUDIT V2.0] Transition: ${transition.fromStatus || 'INITIAL'} → ${transition.toStatus}`);
...
export async function getLastTransition(propostaId: string) {
  try {
    const transitions = await db
      ...
      .where(
        and(
          eq(statusTransitions.propostaId, propostaId),
          isNull(statusTransitions.errorMessage)
        )
      )

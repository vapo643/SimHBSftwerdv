 ...
 */
function calculateMonthlyPaymentRaw(principal: number, monthlyRate: number, numberOfPayments: number): number {
 ...
 */
function calculateMonthlyPaymentDomain(principal: number, monthlyRate: number, numberOfPayments: number): number {
  const rate = monthlyRate / 100;
...
describe('Validação de Equivalência: Domain vs Raw', () => {
  ...
  test('Cenário 2: Valor 50000, Taxa 1.8%, Prazo 24 meses', () => {
    ...
    console.log(`DOMAIN: ${domainResult}, RAW: ${rawResult}, Diff: ${Math.abs(domainResult - rawResult)}`);
  ...
  test('Cenário 5: BENCHMARK - Valores típicos de produção', () => {
    ...
    cenarios.forEach(({ valor, taxa, prazo }, index) => {
      ...
      console.log(`Cenário ${index + 1}: V=${valor}, T=${taxa}%, P=${prazo} | D=${domainResult}, R=${rawResult}, Diff=${difference}`);

...
export function logEnvironmentConfig(config: EnvironmentConfig): void {
  ...
  console.log(`  - Rate Limit: ${config.rateLimitMaxRequests} requests per ${config.rateLimitWindow / 60000} minutes`);
  console.log(`  - CORS Origins: ${config.corsOrigins.join(', ')}`);
  
  // Never log sensitive values
  console.log(`  - Secrets Configured: ✅ (${Object.keys(config).filter(k => k.includes('Secret')).length} secrets)`);
}
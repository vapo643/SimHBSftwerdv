...
async function testGerenteAccess() {
  ...
  try {
    ...
    if (error.response?.status === 403) {
      ...
      console.log(
        `${colors.green}✅ Roles requeridos: ${JSON.stringify(error.response.data.requiredRoles)}${colors.reset}`
      );
      console.log(
...
async function testGetSpecificContract(contractId: string) {
  ...
  try {
    ...
    console.log(
      `${colors.green}✅ Status: ${response.status} - Contrato encontrado${colors.reset}`
    );

...
export function validatePassword(password: string, userInputs: string[] = []): PasswordValidationResult {
  ...
  const characterTypes = [hasUpperCase, hasLowerCase, hasNumber, hasSpecialChar].filter(Boolean).length;
...
export function getPasswordStrengthFeedback(password: string, userInputs: string[] = []): {

      ...
      if (statusContextualExistente) {
        ...
        await tx
          ...
          .set({
            ...
            observacoes,
            metadata: metadata ? sql`${JSON.stringify(metadata)}::jsonb` : statusContextualExistente.metadata
          })
...
export async function validateStatusConsistency(
  ...
  try {
    ...
    const inconsistencias = contextosStatus.filter((cs: any) => {
      ...
      if (cs.contexto === 'cobrancas' && ['QUITADO', 'INADIMPLENTE'].includes(proposta?.status || '')) {
    ...
    if (!isConsistent) {
      console.warn(`[CONSISTÊNCIA] ⚠️ Inconsistências detectadas para proposta ${propostaId}:`, inconsistencias);
    }

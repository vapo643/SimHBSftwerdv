...
export class SecurityMonitor {
   ...
   */
  start(intervalMs: number = 60000): void { // Check every minute
  ...
  private async checkBruteForceAttempts(): Promise<void> {
    ...
    const failedLoginsByIp = await db
      ...
      .where(
        and(
          eq(security_logs.event_type, 'LOGIN_FAILED'),
  ...
  private async checkUnusualAccessPatterns(): Promise<void> {
    ...
    if (currentHour >= ALERT_THRESHOLDS.UNUSUAL_HOUR_START && 
      ...
      for (const record of recentAccess) {
        if (record.count > 10) { // More than 10 data accesses in 5 minutes at unusual hours
          this.createAlert({

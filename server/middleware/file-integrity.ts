 ...
 */
export function fileIntegrityMiddleware(req: FileIntegrityRequest, res: Response, next: NextFunction) {
  ...
  res.send = function(data: any) {
    if (Buffer.isBuffer(data) && res.getHeader('Content-Type')?.toString().includes('application/pdf')) {
...
export function verifyFileIntegrityEndpoint(req: Request, res: Response) {
  try {
    ...
    if (size && size !== storedIntegrity.size) {
      verification.valid = false;
      verification.errors.push(`Tamanho não corresponde: esperado ${storedIntegrity.size}, recebido ${size}`);
    }
    
    // Log verification attempt
    securityLogger.logEvent({
      type: verification.valid ? SecurityEventType.DATA_ACCESS : SecurityEventType.FILE_INTEGRITY_VIOLATION,

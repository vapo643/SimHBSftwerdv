...
async function buscarPropostaComBoletos(token: string): Promise<{propostaId: string, codigoSolicitacao: string}> {
  try {
...
async function testarProrrogacaoComAuditoria(token: string, codigoSolicitacao: string) {
  ...
  try {
    ...
    const response = await axios.patch(`${API_URL}/inter/collections/batch-extend`, {
...
async function testarDescontoComAuditoria(token: string, propostaId: string) {
  ...
  try {
    ...
    const proposalResponse = await axios.get(`${API_URL}/inter/collections/proposal/${propostaId}`, {
      headers: { 
        'Authorization': `Bearer ${token}`,
        'Cookie': `jwt=${token}`
      }
    ...
    const response = await axios.post(`${API_URL}/inter/collections/settlement-discount`, {
      propostaId,
      desconto,
      novasParcelas
    }, {

 ...
 */
export function drawCrosshair(page: PDFPage, x: number, y: number, size: number = 10, color = rgb(1, 0, 0)) {
    // Linha horizontal
    page.drawLine({
        start: { x: x - size, y: y },
        end: { x: x + size, y: y },
 ...
 */
export function getCenteredX(text: string, font: PDFFont, fontSize: number, pageWidth: number): number {
 ...
 */
export function getRightAlignedX(text: string, font: PDFFont, fontSize: number, rightMargin: number): number {
    const textWidth = getTextWidth(text, font, fontSize);
 ...
 */
export async function generateTestCoordinatesPDF(coordinates: Array<{
    page: number;
    x: number;
    y: number;
    label: string;
 ...
 */
export function validateCoordinates(x: number, y: number, pageWidth: number, pageHeight: number): boolean {

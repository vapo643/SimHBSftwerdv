...
export class PropostaMapper {
  ...
  static toViewModel(apiResponse: PropostaApiResponse): PropostaAnaliseViewModel {
    ...
    if (data.cliente_data || data.clienteData) {
      ...
      try {
        clienteData = typeof rawClienteData === 'string' 
          ? JSON.parse(rawClienteData) 
          : rawClienteData;
      } catch (e) {
    ...
    return {
      ...
      cliente: {
        ...
        rendaMensal: this.formatMoney(this.extractValue([
      ...
      condicoes: {
        valorSolicitado: this.formatMoney(this.extractValue([
          data.valor,
        ...
        valorTac: this.formatMoney(this.extractValue([
          data.valor_tac,
          data.valorTac,
          500 // Fallback temporário para teste
        ])),
        valorIof: this.formatMoney(this.extractValue([
          data.valor_iof,
          data.valorIof,
          200 // Fallback temporário para teste
        ])),
        valorTotalFinanciado: this.formatMoney(this.extractValue([

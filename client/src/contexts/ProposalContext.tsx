...
export function useProposalActions() {
  ...
  const clearSimulation = useCallback(() => dispatch({ type: 'CLEAR_SIMULATION' }), [dispatch]);
    ...
    (step: number): boolean => {
      switch (step) {
        case 0: // Dados do Cliente
          ...
          const hasValidationErrors = Object.keys(state.errors).some(
            (key) =>
              key.includes('cpf') ||
              key.includes('email') ||
        ...
        case 1: // Referências Pessoais
          ...
          const allReferencesValid = references.every(
            (ref) =>
              ref.nomeCompleto &&
              ref.nomeCompleto.trim() !== '' &&
              ref.grauParentesco &&
              ref.grauParentesco.trim() !== '' &&
              ref.telefone &&
              ref.telefone.trim() !== ''
          );
        ...
        case 2: // Condições do Empréstimo
          ...
          const loanFieldsValid =
            ...
            loanData.valorSolicitado &&
            loanData.valorSolicitado.trim() !== '' &&
            loanData.prazo !== null;

          // Verificar se não há erros de validação de empréstimo
          const hasLoanErrors = Object.keys(state.errors).some(
            (key) =>

...
// Fetch functions
const fetchProposta = async (id: string) => {
...
const AnaliseManualPage: React.FC = () => {
  ...
  const { control, handleSubmit, register } = useForm<DecisionFormData>({
        ...
        <div className="md:col-span-2 space-y-6">
            <Card>
              ...
              <CardContent>
                ...
                <p><strong>CPF:</strong> {proposta.clienteCpf}</p>
                <p><strong>Valor Solicitado:</strong> R$ {proposta.valorSolicitado?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                <p><strong>Prazo:</strong> {proposta.prazoMeses} meses</p>
                <p><strong>Status:</strong> {proposta.status}</p>
                <p><strong>Data de Criação:</strong> {new Date(proposta.createdAt).toLocaleString('pt-BR')}</p>
              </CardContent>
            </Card>
            <Card>

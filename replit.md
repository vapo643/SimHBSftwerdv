...
- **✅ PAM V1.0 - Tela de Cobranças Implementation (14/08/2025)**: Successfully implemented Collections Screen with specific business rules. Backend endpoint `/api/cobrancas` updated with PAM V1.0 requirements: assinaturaEletronicaConcluida = true AND EXISTS in inter_collections. New frontend page `/cobrancas` created with comprehensive table view showing proposal number, client name, collection status, overdue days, and pending amounts. Implemented three core actions: Ver/Baixar Carnê (generates/downloads merged PDF), Copiar PIX (copies PIX code from first pending installment), Copiar Linha Digitável (copies barcode from first pending installment). Added role-based access control for COBRANCA, FINANCEIRO, and ADMINISTRADOR roles. System displays real-time statistics including total collections, overdue accounts, and total overdue value.

...
- **✅ Two-Phase Carnê Generation Frontend UI (13/08/2025)**: Implemented complete frontend orchestration for the new two-phase carnê generation flow. UI button "Gerar Carnê para Impressão" now executes asynchronous sequence: 1) Synchronizes boletos from Inter to Storage, 2) Generates consolidated carnê from stored PDFs, 3) Automatically downloads the merged document. Features progressive toast notifications for user feedback, loading states with spinner, and robust error handling for each API call. Test scripts validate the complete frontend flow.

...
- **✅ BYPASS SOLUTIONS COMPLETELY PURGED (13/08/2025)**: **MAJOR CLEANUP COMPLETED** - All McAfee bypass solutions, PNG conversion services, alternative format generators, and related workarounds have been systematically removed from the codebase per user requirements. This includes: removal of McAfeeSpecificBypass service, PDF repair mechanisms, PNG conversion endpoints, fragment download systems, advanced options modals, and all related UI components. The formalization interface has been restored to its clean, original state. Frontend PNG buttons (lines 1559-1700) and backend PNG endpoint (/api/inter/collections/:id/png) completely removed. The `/baixar-todos-boletos` endpoint has been preserved and simplified for future pdf-lib fusion implementation. Zero LSP diagnostics remain - codebase is completely clean and ready for the new PDF merge approach using pdf-lib library.

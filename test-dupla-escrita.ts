...
async function testarDuplaEscrita() {
  ...
  try {
    ...
    console.log(`   Tabela Contextual (status_contextuais): ${statusContextual?.status || "NÃO ENCONTRADO"}`);
    
    const isConsistent = 
      propostaAtualizada.status === "TESTE_PAM_V1" &&
      statusContextual?.status === "TESTE_PAM_V1";

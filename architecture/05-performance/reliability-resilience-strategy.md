  ...
  static async executeWithRetry(fn, configType = 'external_api', context = '') {
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        ...
        console.warn(
          `[RETRY] ${context} attempt ${attempt} failed, retrying in ${Math.round(totalDelay)}ms:`,
          error.message
        );
...
class LoadSheddingManager {
  ...
  getRequestPriority(req) {
    ...
    const highPatterns = [/^\/api\/propostas/, /^\/api\/users\/profile/, /^\/api\/dashboard/];

    const mediumPatterns = [/^\/api\/reports/, /^\/api\/analytics/, /^\/api\/files/];
...
const showDegradationNotification = (status) => {
  const messages = {
    YELLOW:
      'Alguns recursos avançados foram temporariamente desabilitados para melhorar a performance.',
    ORANGE:
      'Sistema em modo de performance otimizada. Funcionalidades essenciais permanecem disponíveis.',
...
class ReliabilityMetricsService {
  ...
  calculateMTTR(timeframe = '30d') {
    ...
    return {
      ...
      fastest_recovery:
        Math.min(...incidents.map((i) => i.resolved_at - i.started_at)) / (1000 * 60),
      slowest_recovery:
        Math.max(...incidents.map((i) => i.resolved_at - i.started_at)) / (1000 * 60),
    };
  }
  ...
  calculateErrorBudget(sliType, currentValue) {
    ...
    return {
      ...
      status:
        percentageRemaining > 50 ? 'HEALTHY' : percentageRemaining > 25 ? 'WARNING' : 'CRITICAL',
    };
  }
  ...
  calculateReliabilityScore(mtbf, mttr, sli) {
    ...
    const scores = {
      availability: sli.availability.met
        ? 25
        : Math.max(0, (sli.availability.value / sli.availability.target) * 25),
    ...
    if (experiment.name === 'database_latency') {
      ...
      insights.push({
        ...
        adaptation_suggestion:
          resilience < 0.8 ? 'increase_cache_usage' : 'maintain_current_config',
      });
    }
...
_25/08/2025 - Doutrina de Resiliência Estabelecida_

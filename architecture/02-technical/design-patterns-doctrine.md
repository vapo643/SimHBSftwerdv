...
export class DrizzleStorage implements IStorage {
  ...
  async getPropostasByStatus(status: string): Promise<Proposta[]> {
    return await db
      .select()
      .from(propostas)
      .where(and(eq(propostas.status, status), isNull(propostas.deleted_at)))
...
async function criarPropostaCompleta(dadosCliente: ClienteData): Promise<string> {
  return await db.transaction(async (trx) => {
    try {
      ...
      await trx.insert(parcelas).values(
        cronogramaParcelas.map((parcela) => ({
          ...parcela,
      ...
      await trx.insert(analiseCredito).values({
        propostaId: proposta.id,
...
async function processarPagamentoInter(
  ...
  await db.transaction(async (trx) => {
    ...
    await trx.insert(pagamentos).values({
...
async function processarPagamentoCritico(
  ...
  return await db.transaction(async (trx) => {
    ...
    if (proposta.status !== 'aguardando_pagamento') {
      throw new InvalidStatusError(`Status inválido para pagamento: ${proposta.status}`);
    }
...
export class CacheService {
  ...
  async getWithLock<T>(
    ...
    try {
      ...
      const lockAcquired = await this.redis.set(lockKey, 'locked', 'EX', lockTTL, 'NX');
...
function convertToProblemDetails(
  ...
  return {
    ...
    status: 500,
    detail: process.env.NODE_ENV === 'production' ? 'Erro interno do servidor' : error.message,
    instance: req.url,
    context: {
      correlationId,
      timestamp: new Date().toISOString(),
...
export class StatusFsmService {
  ...
  static async transitionTo(
    ...
    return await db.transaction(async (trx) => {
      ...
      await trx.update(propostas).set({ status: newStatus }).where(eq(propostas.id, propostaId));
...
export class SecurityMonitoringService {
  ...
  static getInstance(
    ...
    if (!this.instance) {
      this.instance = new SecurityMonitoringService(logger, alertService, rateLimitService);
    }
...
export class ServiceFactory {
  ...
  static createFinanceService(): FinanceService {
    return new FinanceService(this.logger, this.storage, this.createTacCalculationService());
  }
  ...
  static createStatusFsmService(): StatusFsmService {
    return new StatusFsmService(this.storage, this.logger);
  }

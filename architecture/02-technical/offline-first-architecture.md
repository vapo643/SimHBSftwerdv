...
class IndexedDBManager {
  ...
  async delete<T extends keyof StorageSchema>(
    storeName: T,
    key: string | number
  ): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async addToSyncQueue(operation: Omit<SyncOperation, 'id' | 'timestamp' | 'retries' | 'status'>): Promise<string> {
...
class SyncQueueManager {
  ...
  private async handleSyncError(operation: SyncOperation, error: Error): Promise<void> {
    ...
    if (retries >= maxRetries) {
      ...
      console.warn(`[SYNC] Operation ${operation.id} failed, retrying in ${backoffMs}ms (attempt ${retries})`);
    }
  }
  ...
  private notifyApp(type: string, data: any): void {
    ...
    window.dispatchEvent(new CustomEvent('simpix-sync', {
      detail: { type, data }
    }));
  }
}
...
class ConflictResolutionEngine {
  ...
  private mergeWithFieldRules(
    local: any,
    server: any,
    fieldRules: FieldResolutionConfig[]
  ): any {
...
const offlineFirstRisks: OfflineRisk[] = [
  {
    ...
    probability: 'HIGH',
    impact: 'HIGH',
    mitigation: 'Implementar estratégias hierárquicas de resolução, testes extensivos, logs detalhados',

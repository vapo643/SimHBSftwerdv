...
async function criarPropostaCompleta(dados: CriarPropostaRequest): Promise<void> {
  await db.transaction(async (trx) => {
    ...
    await trx.insert('parcelas', parcelas.map(p => ({
      ...p,
...
class SagaMonitor {
  ...
  private async checkSagaAlerts(metrics: SagaMetrics): Promise<void> {
    ...
    if (metrics.duration > 5 * 60 * 1000) { // 5 minutos
...
const POINTS_OF_NO_RETURN: PointOfNoReturn[] = [
  {
    ...
    riskLevel: 'critical',
    mitigationStrategy: 'Executar todos os passos seguintes ou notificar cliente sobre inconsistência'
  },
  {
...
*Sprint 1 - Fundação de Dados e Padrões*
...
class AzureContainerAppsRollback {
  async executeRollback(config: ContainerAppRollback): Promise<RollbackResult> {
    try {
      ...
      console.log(`[ROLLBACK] Iniciando rollback de ${currentRevision.name} para ${targetRevision.name}`);
  ...
  private async executeCanaryRollback(config: ContainerAppRollback, targetRevision: any, trafficPercent: number) {
    const body = {
      properties: {
        configuration: {
          ingress: {
      ...
      for (const column of table.columns) {
        ...
        if (newColumn && newColumn.type !== column.type) {
          ...
          if (!isTypeChangeCompatible(column.type, newColumn.type)) {
            breakingChanges.push(`Incompatible type change: ${table.name}.${column.name} ${column.type} -> ${newColumn.type}`);
          }
        }
      }
    }

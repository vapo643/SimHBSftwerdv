...
async function migrateLegacyProductsTac() {
  ...
  try {
    ...
    console.log(`📊 [MIGRAÇÃO TAC] Encontrados ${productsToUpdate.length} produtos sem configuração de TAC`);
    ...
    // Reportar sucesso
    console.log(`✅ [MIGRAÇÃO TAC] Migração concluída. ${updatedProducts.length} produtos legados foram atualizados com a TAC padrão.`);
    
    // Mostrar valores atualizados
    console.log("📊 [MIGRAÇÃO TAC] Valores finais:");
    updatedProducts.forEach(produto => {
      console.log(`   - ID: ${produto.id} | Nome: ${produto.nomeProduto} | TAC: R$ ${produto.tacValor} (${produto.tacTipo})`);
    });

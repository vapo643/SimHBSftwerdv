...
async function cleanupNullProposals() {
  ...
  try {
    ...
    const deleteResult = await db
      .delete(propostas)
      .where(eq(propostas.id, targetProposalId));
...
async function verifyDataIntegrity() {
  ...
  try {
    ...
    console.log(`📈 Taxa de integridade: ${((validPropostas.length / totalPropostas.length) * 100).toFixed(2)}%`);

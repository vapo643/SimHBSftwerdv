...
class MetricsCollector {
  ...
  recordRequest(endpoint, duration, success, statusCode) {
    ...
    if (endpoint.includes('/propostas') && endpoint.split('/').length === 3)
      this.metrics.create_requests++;
...
async function runProposalFlow(userIndex, metricsCollector) {
  try {
    ...
    console.log(
      `🔍 [DEBUG] Token length: ${token.length}, Token prefix: ${token.substring(0, 50)}...`
    );
    ...
    if (!createResponse.success) {
      console.error(
        `❌ [User ${userIndex}] Proposal creation failed - Status: ${createResponse.status}`
      );
      return false;
    }
    ...
    const approveResponse = await makeRequest(
      `${BASE_URL}/api/propostas/${proposalId}/approve`,
      {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    await makeRequest(
      `${BASE_URL}/api/monitoring/queues/metrics`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
...
function generateReport(metricsCollector, totalDuration) {
  ...
  console.log(
    `   Error Rate: ${stats.error_rate}% (Target: 0%) - ${stats.error_rate === 0 ? '✅ PASS' : '❌ FAIL'}`
  );
  console.log(
    `   Iterations: ${stats.iterations}/${TARGET_ITERATIONS} - ${stats.iterations >= TARGET_ITERATIONS ? '✅ COMPLETE' : '⚠️ PARTIAL'}`
  );

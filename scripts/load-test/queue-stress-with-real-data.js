...
async function createRealProposals() {
  ...
  if (proposalIds.length === 0) {
    throw new Error('❌ CRÍTICO: Nenhuma proposta real foi criada! Não é possível testar caminho feliz.');
  }
...
async function validateFinalResults(expectedHappy, expectedFailure) {
  ...
  try {
    ...
    console.log(`   Jobs Esperados: ${expectedTotal} (${expectedHappy} feliz + ${expectedFailure} falha)`);
    ...
    return {
      ...
      metrics: {
        ...
        failedJobs: failed,
        successRate: expectedTotal > 0 ? ((completed / expectedTotal) * 100).toFixed(1) + '%' : '0%',
        processingRate: expectedTotal > 0 ? ((totalJobsProcessed / expectedTotal) * 100).toFixed(1) + '%' : '0%',
      }
    };

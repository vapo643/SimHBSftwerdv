...
async function testService() {
  try {
    ...
    const totalRecommendations = results.reduce((sum, cs) => 
      sum + (cs.recommendations?.length || 0), 0);
    const implemented = results.reduce((sum, cs) => 
      sum + (cs.recommendations?.filter(r => r.currentStatus === 'implemented').length || 0), 0);
    const partial = results.reduce((sum, cs) => 
      sum + (cs.recommendations?.filter(r => r.currentStatus === 'partial').length || 0), 0);
    const notImplemented = results.reduce((sum, cs) => 
      sum + (cs.recommendations?.filter(r => r.currentStatus === 'not_implemented').length || 0), 0);
    const notApplicable = results.reduce((sum, cs) => 
      sum + (cs.recommendations?.filter(r => r.currentStatus === 'not_applicable').length || 0), 0);
    ...
    console.log(`1. ${results[0]?.name} - ${results[0]?.recommendations?.length || 0} recomendações`);
    console.log(`10. ${results[9]?.name} - ${results[9]?.recommendations?.length || 0} recomendações`);
    console.log(`50. ${results[49]?.name} - ${results[49]?.recommendations?.length || 0} recomendações`);
    console.log(`100. ${results[99]?.name} - ${results[99]?.recommendations?.length || 0} recomendações`);
    console.log(`111. ${results[110]?.name} - ${results[110]?.recommendations?.length || 0} recomendações`);

...
async function validateStatusConsistency(): Promise<ValidationResult> {
  ...
  try {
    ...
    console.log(
      `  ✓ Encontradas ${colors.bright}${allPropostas.length}${colors.reset} propostas\n`
    );
    ...
    console.log(
      `  ✓ Encontrados ${colors.bright}${allContextuais.length}${colors.reset} registros contextuais\n`
    );
    ...
    console.log(
      `  • Taxa de Cobertura: ${colors.bright}${((totalComContexto / allPropostas.length) * 100).toFixed(2)}%${colors.reset}`
    );
    ...
    if (orfasRecentes.length > 0) {
      ...
      console.log(
        `${colors.yellow}${orfasRecentes.length} propostas criadas após 19/08/2025 sem status contextual:${colors.reset}`
      );

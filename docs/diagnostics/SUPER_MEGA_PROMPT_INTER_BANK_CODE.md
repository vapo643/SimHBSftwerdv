# Super Mega Prompt de Socorro - Código Completo Inter Bank API

## Contexto do Problema

Estamos integrando com a API v3 do Banco Inter para geração de boletos/PIX mas recebendo erro 400 persistente. Já tentamos múltiplas correções incluindo:

- Mudança entre v2 e v3 da API
- Adição de todos campos obrigatórios (incluindo formasRecebimento)
- Correção de formatação de certificados
- Implementação de desconto obrigatório
- Debug extensivo de autenticação OAuth2

## 1. SERVIÇO COMPLETO - server/services/interBankService.ts (934 linhas)
...
class InterBankService {
      ...
      if (data && (method === 'POST' || method === 'PATCH' || method === 'PUT')) {
        ...
        const customStringify = (obj: any): string => {
          const json = JSON.stringify(obj, (key, value) => {
            ...
            if (
              (key === 'valorNominal' || key === 'taxa' || key === 'valor') &&
              typeof value === 'number'
            ) {
  ...
  async emitirCobranca(cobrancaData: CobrancaRequest): Promise<CobrancaResponse> {
    try {
      ...
      console.log(
        `[INTER]   - valorNominal: ${cobrancaData.valorNominal} type: ${typeof cobrancaData.valorNominal}`
      );
...
## 2. ROTAS PRINCIPAIS - server/routes/inter.ts (628 linhas)
...
router.post('/collections', jwtAuthMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log(
      `[INTER] ✅ Collection created successfully: ${collectionResponse.codigoSolicitacao}`
    );
 ...
 */
router.post(
  '/collections/:codigoSolicitacao/simulate-payment',
  jwtAuthMiddleware,
  async (req: AuthenticatedRequest, res) => {
    try {
      const { codigoSolicitacao } = req.params;
...
## 3. ROTAS DE COLEÇÕES - server/routes/inter-collections.ts (171 linhas)
...
## INFORMAÇÕES CRÍTICAS PARA DEBUG

### URL da API

- Produção: `https://cdpj.partners.bancointer.com.br`
- Versão: v3 (confirmada pela documentação oficial)
- Endpoint de cobrança: `/cobranca/v3/cobrancas`

### Autenticação

- OAuth2 com mTLS
- Grant type: `client_credentials`
- Scopes: `boleto-cobranca.read boleto-cobranca.write webhook.read webhook.write`

### Erro Atual

- Status: 400 Bad Request
- Body: vazio (0 caracteres)
- Headers de resposta incluem `traceparent` mas sem detalhes do erro

  ...
  db: {
    select: vi.fn().mockReturnValue({
      from: vi.fn().mockReturnValue({
        innerJoin: vi.fn().mockReturnValue({  // ← ADICIONAR ESTE MÉTODO
...
await request(app)
  ...
  .send({ status: 'aprovado' })  // ← Body ESTÁ sendo enviado
```
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      const response = await request(app)
        .put(endpoint)
        .send({ status: 'aprovado' })
        .timeout(5000);  // Timeout menor e explícito
...
*Relatório gerado automaticamente pelo Sistema de Diagnóstico PAM V1.0*  
*Protocolo PACN V1.0 - Auditoria de Cenário de Negócio Validada*
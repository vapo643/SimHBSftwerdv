...
describe("TAC Integration Tests - Authenticated & Validated", () => {
  ...
  beforeEach(async () => {
    ...
    console.log(`[TAC AUTH] 🔗 Using existing test user: ${existingTestUser.email} (ID: ${testUserId})`);
  ...
  describe("Cenário 1: Cliente Novo Paga TAC (Autenticado)", () => {
    it("deve calcular TAC fixa R$ 180,00 para cliente novo via API autenticada", async () => {
      ...
      expect(valorTacBanco).toBe(180.00);
    ...
    it("deve calcular TAC percentual 2.5% para cliente novo via API autenticada", async () => {
      ...
      expect(valorTacBanco).toBe(500.00);
  ...
  describe("Cenário 2: Cliente Cadastrado Isento de TAC (Autenticado)", () => {
    it("deve isentar TAC para cliente com proposta anterior quitada", async () => {
      ...
      expect(primeiroTac).toBe(350.00);
      ...
      console.log("[TAC AUTH] ⏳ Primeira proposta marcada como QUITADA (cliente agora cadastrado)");
  ...
  describe("Cenário 3: Validação de Integridade do Sistema", () => {
    it("deve manter consistência entre API response e banco de dados", async () => {
      ...
      expect(tacResponse).toBe(250.00);
    ...
    it("deve processar múltiplas propostas sequenciais sem conflito", async () => {
      ...
      const propostasData = [
        { ...baseProposal, valor: 10000, clienteNome: "Cliente 1", clienteCpf: "11111111111", clienteEmail: "cliente1@teste.com" },
        { ...baseProposal, valor: 15000, clienteNome: "Cliente 2", clienteCpf: "22222222222", clienteEmail: "cliente2@teste.com" },
        { ...baseProposal, valor: 20000, clienteNome: "Cliente 3", clienteCpf: "33333333333", clienteEmail: "cliente3@teste.com" }
      ];

      const tacesEsperadas = [300.00, 450.00, 600.00]; // 3% de cada valor

...
describe('userService - createUser', () => {
  ...
  describe('Cenário de Falha - Email Duplicado', () => {
    it('deve lançar ConflictError quando email já existe', async () => {
      ...
      // Act & Assert
      await expect(createUser(userData)).rejects.toThrow('Usuário com email duplicado@exemplo.com já existe.');
  ...
  describe('Cenário de Sucesso - Rollback', () => {
    it('deve executar rollback quando falha na criação do profile', async () => {
      ...
      // Act & Assert
      await expect(createUser(userData)).rejects.toThrow('Erro ao criar perfil: Database constraint violation');
  ...
  describe('Cenários de Falha no Auth', () => {
    it('deve falhar quando Supabase Auth retorna erro', async () => {
      ...
      // Act & Assert
      await expect(createUser(userData)).rejects.toThrow('Erro no Supabase Auth: Auth service unavailable');
    });
  ...
  describe('Cenário de Falha - Gerente Lojas Association', () => {
    it('deve executar rollback quando falha na criação das associações gerente_lojas', async () => {
      ...
      // Act & Assert
      await expect(createUser(userData)).rejects.toThrow('Erro ao associar gerente a lojas: Foreign key constraint failed');

...
import { ProposalStatus } from "@shared/schema";
...
describe("Status FSM Unit Tests", () => {
  ...
  describe("Scenario 1: Valid Transitions", () => {
    ...
    it("should allow transition from APROVADO to AGUARDANDO_DOCUMENTACAO", () => {
      const isValid = validateTransition(ProposalStatus.APROVADO, ProposalStatus.AGUARDANDO_DOCUMENTACAO);
      expect(isValid).toBe(true);
    });
    ...
    it("should allow multiple valid transitions in sequence", () => {
      ...
      expect(validateTransition(ProposalStatus.APROVADO, ProposalStatus.AGUARDANDO_DOCUMENTACAO)).toBe(true);
      
      // AGUARDANDO_DOCUMENTACAO -> DOCUMENTACAO_COMPLETA
      expect(validateTransition(ProposalStatus.AGUARDANDO_DOCUMENTACAO, ProposalStatus.DOCUMENTACAO_COMPLETA)).toBe(true);
      
      // DOCUMENTACAO_COMPLETA -> ASSINATURA_PENDENTE
      expect(validateTransition(ProposalStatus.DOCUMENTACAO_COMPLETA, ProposalStatus.ASSINATURA_PENDENTE)).toBe(true);
    });
  });
  ...
  describe("Scenario 2: Invalid Transitions", () => {
    ...
    it("should reject backward transition from PAGO_TOTAL to AGUARDANDO_PAGAMENTO", () => {
      const isValid = validateTransition(ProposalStatus.PAGO_TOTAL, ProposalStatus.AGUARDANDO_PAGAMENTO);
      expect(isValid).toBe(false);
    });
  });

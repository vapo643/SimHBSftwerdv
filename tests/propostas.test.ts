...
async function cleanupTestData() {
  ...
  await adminClient.from('propostas').delete().ilike('id', 'PROP-TEST-%');

  // Limpar logs de teste
  await adminClient.from('proposta_logs').delete().ilike('proposta_id', 'PROP-TEST-%');
}
...
describe('Propostas API', () => {
  ...
  afterAll(async () => {
    ...
    if (testLojaId) {
      await adminClient.from('lojas').delete().eq('id', testLojaId);
    }
  });
  ...
  describe('POST /api/propostas - Criação de Proposta', () => {
    ...
    it('deve retornar erro 401 sem autenticação', async () => {
      const response = await request(app).post('/api/propostas').send({});
  ...
  describe('GET /api/propostas - Listagem com Filtros', () => {
    beforeEach(async () => {
      ...
      for (const status of statuses) {
        await adminClient.from('propostas').insert({
